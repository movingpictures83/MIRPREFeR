...
def get_complement(seq):
    ...
    trans_table = str.maketrans("ATGCU","UACGA")
...
def dump_loci_seqs_and_alignment_multiprocess(dict_loci, piece_info_list,
    ...
    def dump_piece(dict_loci, piece_info_list,sortedbamname, fastaname,
        ...
        foutdump = open(outputalnname,'wb')
        dump_key = None
        num_seq = 0
        num_loci = 0
            ...
            for loci in dict_loci[seqid][start:end]:
                ...
                for idx, extendregion in enumerate(loci[1:]):
                    ...
                    if len(strands) == 1:
                        ...
                        pickle.dump([dump_key, extendregiontag.strip(), dict_loci_info, matures], foutdump, protocol=2)
                        for m in matures:
                    ...
                    if len(strands) == 2:  # this region has peaks on both strands, generate precursors for each strand
                        ...
                        if len(plus_tag):
                            for which, tag in enumerate(plus_tag):
                                fout.write(tag)
                                fout.write(plus_seq[which]+"\n")
                                pickle.dump(plus_dumpinfo[which], foutdump, protocol=2)
                            for which, tag in enumerate(minus_tag):
                                fout.write(minus_tag[which])
                                fout.write(minus_seq[which]+"\n")
                                pickle.dump(minus_dumpinfo[which], foutdump, protocol=2)
        fout.close()
        foutdump.close()
...
def samtools_view_region(sortedbamname, seqid, start, end):
    ...
    try:
        ...
        sys.exit(-1)
    alignments = outmessage.decode('utf-8').strip().split("\n")
    if alignments[0]:
        alignments.pop()
        return alignments
...
def gen_possible_matures_loci(dict_loci_info, loci, strand, min_mature_depth):
    def gen_matures_one_peak(max_num_mature, min_mature_depth, peak_start, peak_end):
        ...
        matures = []
        higest = (0, 0, 0, 0)
        for pos in range(peak_start, peak_end):
...
def get_maturestar_info(ss, mature, foldstart, foldend, regionstart, regionend,
    ...
    mature_ss = ss[int(mature_local_pos[0]):int(mature_local_pos[1])]
    ...
    mature_local_pos = (int(mature_local_pos[0]), int(mature_local_pos[1]))
    #  local positions
    star_start = 0
    star_end = 0
    star_ss = ""
...
def filter_next_loci(alndumpname, rnalfoldoutname, combinedsortedbamname,
    ...
    while True:
        ...
        matures = None
        try:
            region, which, dict_aln, matures = pickle.load(alnf)
        except EOFError:
            raise StopIteration
        ...
        if which == "0": #only one extend region for this loci
            ...
            region1, which1, dict_aln1, matures1 = pickle.load(alnf)
    ...
    for chrom in dict_reasons:  # key is chromosome
        for strand in dict_reasons[chrom]:
            for region in dict_reasons[chrom][strand]:
                ...
                outf.write("\n")
    outdump = open(faileddumpname, 'wb')
    pickle.dump(dict_express_info_all, outdump)
...
def gen_mirna_fasta_ss_from_result(resultlist, maturename, stemloopname,
    ...
    for idx, mirna in enumerate(resultlist):
        ...
        stemloopseq = seqs[3].upper().replace("T", "U")
        structure = mirna[-4]
        maturestart = int(mirna[3] - mirna[1])
        matureend = int(mirna[4] - mirna[1])
        starstart = int(mirna[5] - mirna[1])
        starend = int(mirna[6] - mirna[1])
...
def fold_use_RNALfold(inputfastalist, tempfolder, dict_option, maxspan, chunksize):
    ...
    def fold(inputname, outputname, tempfolder, recovername, chunksize):
        ...
        try:
            recoverfile = open(recovername, 'rb')
            dict_recover_infold = pickle.load(recoverfile)
            recoverfile.close()
            ...
            while next_chunk:
                ...
                recoverfile_temp = open(recovername_temp, 'wb')
                pickle.dump(dict_recover_infold, recoverfile_temp)
...
def previous_stage_saved(recovername, stage):
    if not os.path.exists(recovername):
        return False
    recoverfile = open(recovername, 'rb')
    dict_recover = pickle.load(recoverfile)
...
def run_prepare(dict_option, outtempfolder, recovername, dict_recover):
    logger = None
    if dict_option["LOG"]:
    ...
    samplenamefile = open(samplenamefilename, 'wb')
    pickle.dump(samplenames, samplenamefile)
    ...
    recoverfile = open(recovername,'wb')
    pickle.dump(dict_recover,recoverfile)
    if logger:
...
def run_candidate(dict_option, outtempfolder, recovername):
    ...
    pickle.dump(dict_loci, loci_dump_file, protocol=2)
...
def run_fold(dict_option, outtempfolder, recovername, tryrecover):
    ...
    for name in sorted(dict_recover["finished_stages"]["candidate"]["fasta"]):
        ...
        if dict_recover_fold and tryrecover:
            ...
            foldrecoverfile = open(foldrecovername_temp,'wb')
            pickle.dump(dict_recover_fold, foldrecoverfile)
...
def run_predict(dict_option, outtempfolder, recovername):
    ...
    samplenames = pickle.load(open(samplenamefilename, 'rb'))
...
import PyIO
import PyPluMA
class MIRPREFeRPlugin:
 def input(self, inputfile):
    self.configfile = inputfile
 def run(self):
    pass
 def output(self, outputfile):
    dict_option = parse_configfile(self.configfile)
    dict_option['ACTION'] = "pipeline"
    dict_option["LOG"] = True
    dict_option["KEEPTMP"] = True
    dict_option['OUTPUT_DETAILS_FOR_DEBUG'] = False

    #parse_option_optparse()
    ...
    if dict_option['ACTION'] == 'pipeline':
        ...
        run_prepare(dict_option, outtempfolder, recovername, dict_recover)
        run_candidate(dict_option, outtempfolder, recovername)
        run_fold(dict_option, outtempfolder, recovername, False)
        run_predict(dict_option, outtempfolder, recovername)
        if not dict_option["KEEPTMP"]:
